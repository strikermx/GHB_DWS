...
if __name__ == '__main__':
    ...
    #con.commit()
    #cursor.close()
    SBABook.duplicate = SBABook.duplicate.reset_index(drop=True)
    for i in range(SBABook.duplicate.shape[0]):
    #for i in range(0, 100):    
        sql_2 = '''
        INSERT INTO res_dws_03a_result 
		values('{3}', '{4}', '{5}', '{6}', '{7}', '{8}', '{9}', '{0}', TO_DATE('{1}','{2}'))
        
        '''.format(job_id, job_start_date_time, g_date_format,SBABook.duplicate.SOL_DESC[i],SBABook.duplicate.SOL_ID[i],SBABook.duplicate.FORACID[i],SBABook.duplicate.CIF_ID[i],SBABook.duplicate.ACCT_NAME[i],SBABook.duplicate.SCHM_CODE[i],SBABook.duplicate.BEGIN_CHQ_NUM[i])
        db1.update_rows(sql_2)
        if i%20 == 0:
            print "Done: "+ str(i)
	end = timer()
    transITC.elaspedTime = end - start
    print( "Checking SBA Completed, Time Elapsed : " + str(transITC.elaspedTime) + " seconds")
    # STAMP FINISH JOB
    sql = "UPDATE JOB_TRN_BACKEND \
           SET STATUS='S' \
             , PROGRESS='100' \
