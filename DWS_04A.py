...
if __name__ == '__main__':
    ...
    WTFLAG = DataValidation(pd.DataFrame(), "SOL_ID,FORACID,CIF_ID,ACCT_NAME,WTAX_PCNT,WTAX_FLG,CUST_ID,SCHM_CODE","tbaadm","GAM")
    ...
    WTFLAG.missFlag = pd.merge(WTFLAG.missFlag,SOL.df,on = ['SOL_ID'],how = 'left')
    #75%
    ...
    #cursor.executemany(sql, WTFLAG.missFlag.values.tolist())
    #con.commit()
    #cursor.close()
    WTFLAG.df = WTFLAG.df.reset_index(drop=True)
    for i in range(WTFLAG.df.shape[0]):
    #for i in range(0, 100):    
        sql_2 = '''
        INSERT INTO res_dws_04b_result 
		values('{3}', '{4}', '{5}', '{6}', '{7}', '{8}', '{9}', '{0}', TO_DATE('{1}','{2}'))
        
        '''.format(job_id, job_start_date_time, g_date_format,WTFLAG.df.SOL_DESC[i],WTFLAG.df.SOL_ID[i],WTFLAG.df.FORACID[i],WTFLAG.df.CIF_ID[i],WTFLAG.df.ACCT_NAME[i],WTFLAG.df.WTAX_PCNT[i],WTFLAG.df.WTAX_FLG[i])
