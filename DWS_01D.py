...
def rearrange_int_pd_date():
    ...
    for i in range(df_htd.shape[0]):
         ...
         if((df_htd.VALUE_DATE[i].month == 6 or df_htd.VALUE_DATE[i].month == 12) and df_htd.VALUE_DATE[i].day == 29):
...
def calculate_interest(money, rate, start_date, end_date):
    ...
    if(((start_date.month == 6 or start_date.month == 12) and start_date.day == 29)):
        if(start_date_int_pd):
            start_date = start_date + datetime.timedelta(days=1)
         
         # remove date 29/6 and 29/12 from the end date by backward one day
    if(not((end_date.month == 6 or end_date.month == 12) and end_date.day == 29 and end_date_int_pd)):  
    ...
    if(check_special_int_overlap(start_date, end_date)):
        return calculate_special_int_overlap(money, rate, start_date, end_date)

    elif(check_cross_year(start_date, end_date)):
...
def run_int_adm(df):
    ...
    for i in range(df.shape[0]):
        ...
        if(date.year != 2007):
            ...
            if((((date.month == 6 or date.month == 12) and date.day == 29) and not(next_date.month == date.month and next_date.day == date.day)) or (((date.month == 6 or date.month == 12) and date.day == 29) and i == df.shape[0]-1)):
...
def run_htd():
    ...
    for i in range(df_htd.shape[0]):   
        ...
        if(df_htd.DEL_FLG[i] != 'Y'):
            ...
            if((this_date != last_date) or (((last_date.month == 6 or last_date.month == 12) and last_date.day == 29) and end_date_int_pd)):
...
def run_service(db1, job_id, job_start_date_time, df_input):
    ...
    for i in range(df_input.shape[0]):
        try:
            ...
            if(res != 0):
                sql = '''
                    INSERT INTO RES_DWS_01D_RESULT
...
if __name__ == '__main__':
    ...
    sql = 'select * from int_dws_01d_input'
